...
AFRAME.registerComponent('outline-text', {
	...
	parseData: function(dataString) {
		const data = AFRAME.utils.styleParser.parse(dataString);
	...
	drawText: function(data) {
		this.getTextShape(data).then((shapes) => {
			var geometry = this.getTextGeometry(shapes, data);
			var text = this.getTextMesh(geometry, data);
			var lineText = this.getLineMesh(shapes, geometry, data);

			this.el.setObject3D('text_mesh', text);
			this.el.setObject3D('text_outline', lineText);
		});
	},

	getFontURL: (fontString) => {
	 ...
	 **/
	getTextShape: function(data) {
		const fontUrl = this.getFontURL(data.font);
	 ...
	 **/
	getTextGeometry: function(shapes, data) {
	 ...
	 **/
	getTextMesh: function(geometry, data) {
	 ...
	 **/
	getLineMesh: function(shapes, geometry, data) {
